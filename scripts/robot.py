        ...
        if data.status:
            if data.status.status == ABORTED:
                if data.status.goal_id.id == id_1:
                    ...
                    pu.log_msg(self.robot_id, "Arrived at frontier!!", self.debug_mode)
                    self.initiate_exploration()

                elif data.status.goal_id.id == id_4:
                    self.tried_rendezvous.append(self.rendezvous_point)
                    next_rendezvous = [p for p in self.rendezvous_points if p not in self.tried_rendezvous]
                    if next_rendezvous:
                        pu.log_msg(self.robot_id, "First rendezvous failed...Trying another one now", self.debug_mode)
                        self.rendezvous_point = next_rendezvous[0]
                        self.move_robot_to_goal(self.rendezvous_point, TO_RENDEZVOUS)
                    else:
                        pu.log_msg(self.robot_id, "All rendezvous are  inaccessible...resuming exploration",
                                   self.debug_mode)
                        del self.tried_rendezvous[:]
                        self.heading_back = 0
