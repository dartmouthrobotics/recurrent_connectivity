...
class Graph:
    def __init__(self, robot_id=-1):
        ...
        rospy.Service('/robot_{}/rendezvous_points'.format(self.robot_id), RendezvousPoints,
    ...
    def fetch_rendezvous_points_handler(self, origin, range_val, count=10):
        rendezvous_points = []
        start = time.time()
        ...
        free_points = [p for p in all_points if
                       self.pixel_desc[p] == FREE and pu.D(origin, pu.scale_down(p)) <= range_val]
        if free_points:
        ...
        return RendezvousPointsRespoinse(poses=rendezvous_points)
